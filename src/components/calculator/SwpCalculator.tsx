  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="lg:col-span-4 space-y-6">
          ...
          <CardContent className="px-0 space-y-8">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-primary">{formatCurrency(initial)}</span>
              </div>
              <Slider
                value={[initial]}
                min={100000}
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-primary">{formatCurrency(withdrawal)}</span>
              </div>
              <Slider
                value={[withdrawal]}
                min={500}
        ...
        <Card className="calculator-card bg-secondary text-secondary-foreground">
          <CardContent className="pt-6 space-y-4">
            <div className="flex justify-between items-center border-b border-secondary-foreground/20 pb-2">
              ...
              <span className="font-semibold">{formatCurrency(result.totalWithdrawn)}</span>
            </div>
            <div className="flex justify-between items-center pt-2">
              <span className="text-lg font-bold">Final Balance</span>
              <span className="text-xl font-bold">{formatCurrency(result.finalBalance)}</span>
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <ChartContainer config={{
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={result.yearlyData}>
                  ...
                  <Area type="monotone" dataKey="balance" stroke="hsl(var(--primary))" strokeWidth={3} fillOpacity={0.2} fill="hsl(var(--primary))" />
                  <Area type="monotone" dataKey="withdrawn" stroke="hsl(var(--secondary))" strokeWidth={3} fillOpacity={0.1} fill="hsl(var(--secondary))" />
                </AreaChart>
              </ResponsiveContainer>
            </ChartContainer>
            <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground p-3 bg-muted/30 rounded-lg">
